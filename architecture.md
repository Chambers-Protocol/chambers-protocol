...
System Version: v2.1.0 (Gold Master) Architecture Type: Hybrid Probabilistic-Deterministic Cognitive Engine Protocol: Model Context Protocol (MCP) via Local Pipe
...
Function: It is a Monolithic Server running on Port 8000 (standard IO pipe). It imports all 10 kernels and exposes them as 10 distinct tools to the MCP client.
...
1. The Invocation
The user asks: "Audit the risk of Project X." Claude recognizes the intent and constructs a JSON payload: { "tool": "risk_audit", "args": { "alpha_integrity": 0.8, ... } }

2. The Verification
The Grid Controller receives the JSON.

Step 1: It extracts the CHAMBERS_API_KEY from the local environment.

Step 2: It pings Supabase to pay the "Computation Tax."

Step 3: If Billing Fails -> It returns "⛔ Insufficient Credits" (Computation Aborted).

Step 4: If Billing Succeeds -> It passes the data to RiskMechanicsEngine.
...
Claude receives the raw number (0.64). Claude uses its language capabilities to explain what that number means in the context of the user's question.
